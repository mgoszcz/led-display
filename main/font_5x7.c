...
#define GLYPH_EMPTY {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}

static const font_5x7_glyph_t s_font_5x7[128] = {
    [' '] = GLYPH_EMPTY,
...
const font_5x7_glyph_t *font_5x7_get_glyph(char c)
{
    ...
    const font_5x7_glyph_t *glyph = &s_font_5x7[index];
    if (index != ' ' && (*glyph)[0] == 0x00 && (*glyph)[1] == 0x00 &&
        (*glyph)[2] == 0x00 && (*glyph)[3] == 0x00 && (*glyph)[4] == 0x00 &&
        (*glyph)[5] == 0x00 && (*glyph)[6] == 0x00) {
        return &s_font_5x7['?'];
    }
...
{
    if (glyph == NULL || x >= FONT_5X7_WIDTH || y >= FONT_5X7_HEIGHT) {
        return false;
    }

    uint8_t row = (*glyph)[y];
    uint8_t mask = 1 << (FONT_5X7_WIDTH - 1 - x);
    return (row & mask) != 0;
}
